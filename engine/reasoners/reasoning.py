...
class ReasoningSelector:
    ...
    def select_rules(self, profile: DataProfile) -> List[str]:
        """基于数据特征选择合适的推理规则"""
        features = set()
        if profile.has_numeric: features.add("has_numeric")
        if profile.has_timestamps: features.add("has_timestamps")
        if profile.has_inf_chains: features.add("has_inf_chains")
        if profile.has_dichotomy: features.add("has_dichotomy")
        if profile.has_high_confidence: features.add("has_high_confidence")
        if profile.fact_count > 0: features.add("fact_count")
        if profile.inf_count > 0: features.add("inf_count")
