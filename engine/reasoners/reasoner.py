...
import re
from .reasoner_utils import detect_domain, char_overlap, comparable, extract_prefix, guess_parent, closest_known, project_profile, cosine_similarity
...
class RuleReasoner:
    ...
    def _subsumption_check(self, inferences, facts):
        ...
        for id_str in all_ids:
            prefix = extract_prefix(self, id_str)
        ...
        for prefix, count in sorted(unknown_prefixes.items(), key=lambda x: -x[1]):
            if count >= 2:  # 至少出现2次才建议
                # 猜测最可能的父类型
                guess = guess_parent(self, prefix, type_map)
                results.append(
                    {
                        "type": "subsumption",
                ...
                closest = closest_known(self, prefix)
                results.append(
                    {
                        "type": "subsumption",
    ...
    def _numeric_derive(self, facts):
        ...
        for i in range(len(ids)):
            for j in range(i + 1, len(ids)):
                ...
                # 跳过跨维度比较 (人数vs金额vs百分比)
                if not comparable(self, a["label"], b["label"], a["value"], b["value"]):
                    continue
    ...
    def case_based_reasoning(self, current_project, reference_cases):
        ...
        # 构建当前项目的特征向量
        current_profile = project_profile(self, current_project)

        for case in reference_cases:
            case_profile = project_profile(self, case)
            similarity = cosine_similarity(self, current_profile, case_profile)
            if similarity > 0.3:
                results.append(
                    {
                        "type": "case_match",
