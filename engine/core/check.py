...
def run_check(root, facts_dir, entities_dir, inferences_dir, scheme_dir, log_dir):
    ...
    result(
        ...
        f"推论文件: {len(all_md(inferences_dir))} 个",
    ...
    result(
        "C-07", "实体ID合规性(v2.2 TypeValidator)", len(type_errors) == 0, "WARN" if type_errors else "PASS", detail_c7
    )
