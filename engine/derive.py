...
def main():
    import argparse
    parser = argparse.ArgumentParser(description=f"OntoDerive v{VERSION}")
    parser.add_argument("--init", type=str, help="初始化新项目")
    parser.add_argument("--project", type=str, default=".", help="项目路径")
    parser.add_argument("--derive", action="store_true", help="正向推导")
    parser.add_argument("--check", action="store_true", help="规约检查")
    parser.add_argument("--resolve", action="store_true", help="自动修复")
    parser.add_argument("--rounds", type=int, default=0, help="多轮迭代")
    ...
    if args.init:
        ...
        (root / "facts" / "data.md").write_text("| 编号 | 数据 | 数值 | 来源 |\n|------|------|------|------|\n| D-F1 | 示例数据 | 100 | 来源 |\n")
        (root / "README.md").write_text(f"# {args.init}\n\n由 OntoDerive v{VERSION} 初始化\n")
        print(f"✅ 项目 '{args.init}' 已初始化")
        return
